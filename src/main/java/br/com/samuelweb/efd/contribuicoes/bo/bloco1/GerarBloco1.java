...
public class GerarBloco1 {
	...
	public static StringBuilder gerar(EfdContribuicoes efdContribuicoes, StringBuilder sbr) {
		...
		if (!Util.isEmpty(bloco1.getRegistro1010())) {
			bloco1.getRegistro1010().stream().forEach(registro1010 -> {
				sb = GerarRegistro1010.gerar(registro1010, sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1020())) {
			bloco1.getRegistro1020().stream().forEach(registro1020 -> {
				sb = GerarRegistro1020.gerar(registro1020, sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1100())) {
			bloco1.getRegistro1100().stream().forEach(registro1100 -> {
				...
				if (!Util.isEmpty(registro1100.getRegistro1101())) {
					registro1100.getRegistro1101().stream().forEach(registro1101 -> {
						sb = GerarRegistro1101.gerar(registro1101, sb);
						efdContribuicoes.getContadoresBloco1().incrementar(Bloco1Enum.Registro1101);

						// REGISTRO1102
						if (!Util.isEmpty(registro1101.getRegistro1102())) {
							sb = GerarRegistro1102.gerar(registro1101.getRegistro1102(), sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1200())) {
			bloco1.getRegistro1200().stream().forEach(registro1200 -> {
				...
				if (!Util.isEmpty(registro1200.getRegistro1220())) {
					registro1200.getRegistro1220().stream().forEach(registro1220 -> {
						sb = GerarRegistro1220.gerar(registro1220, sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1500())) {
			bloco1.getRegistro1500().stream().forEach(registro1500 -> {
				...
				if (!Util.isEmpty(registro1500.getRegistro1501())) {
					registro1500.getRegistro1501().stream().forEach(registro1501 -> {
						sb = GerarRegistro1501.gerar(registro1501, sb);
						efdContribuicoes.getContadoresBloco1().incrementar(Bloco1Enum.Registro1501);

						// REGISTRO1502
						if (!Util.isEmpty(registro1501.getRegistro1502())) {
							sb = GerarRegistro1502.gerar(registro1501.getRegistro1502(), sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1600())) {
			bloco1.getRegistro1600().stream().forEach(registro1600 -> {
				...
				if (!Util.isEmpty(registro1600.getRegistro1610())) {
					registro1600.getRegistro1610().stream().forEach(registro1610 -> {
						sb = GerarRegistro1610.gerar(registro1610, sb);
				...
				if (!Util.isEmpty(registro1600.getRegistro1620())) {
					registro1600.getRegistro1620().stream().forEach(registro1620 -> {
						sb = GerarRegistro1620.gerar(registro1620, sb);
		...
		if (!Util.isEmpty(bloco1.getRegistro1800())) {
			bloco1.getRegistro1800().stream().forEach(registro1800 -> {
				sb = GerarRegistro1800.gerar(registro1800, sb);
				efdContribuicoes.getContadoresBloco1().incrementar(Bloco1Enum.Registro1800);

				// REGISTRO1809
				if (!Util.isEmpty(registro1800.getRegistro1809())) {
					registro1800.getRegistro1809().stream().forEach(registro1809 -> {
						sb = GerarRegistro1809.gerar(registro1809, sb);
